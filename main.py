...
def carregar_base_multitipologia_padrao():
    dados = [
        (450000.0, 6000.0, 120.0, 1200.0, 200.0, 2.0, 2.0, 5.0, "CASA"),
        (480000.0, 6153.0, 125.0, 1250.0, 220.0, 2.0, 2.0, 7.0, "CASA"),
        (510000.0, 6375.0, 130.0, 1300.0, 250.0, 2.0, 2.0, 4.0, "CASA"),
        (750000.0, 8823.0, 185.0, 3200.0, 360.0, 3.0, 3.0, 2.0, "CASA"),
        (350000.0, 5833.0, 60.0, 1500.0, 0.0, 1.0, 2.0, 3.0, "APARTAMENTO"),
        (380000.0, 6129.0, 62.0, 1600.0, 0.0, 1.0, 2.0, 6.0, "APARTAMENTO"),
        (420000.0, 6461.0, 65.0, 1800.0, 0.0, 2.0, 3.0, 8.0, "APARTAMENTO"),
        (250000.0, 694.0, 0.0, 1000.0, 360.0, 2.0, 2026.0, 12.0, "LOTE"),
        (1200000.0, 2400.0, 500.0, 900.0, 1000.0, 2.0, 1.0, 10.0, "GALPAO")
    ]
    return pd.DataFrame(dados, columns=[
        'valor_total_declarado', 'valor_unitario_m2', 'area_privativa', 'indice_fiscal', 
        'area_terreno', 'vagas_garagem', 'andares', 'pe_direito', 'tipologia'
    ])
...
def gerar_laudo_pdf_ia(tenant, tipologia, area, valores, model_stats, status_juridico, score_juridico, grafico_buf, equacao):
    ...
    story.append(Paragraph("LAUDO TÉCNICO DE ENGENHARIA DE AVALIAÇÕES (ABNT NBR 14653)", title_style))
    ...
    t2 = Table([
        ...
        ["Margem Mínima de Segurança (Garantia LTV)", f"R$ {valores['v_min']:,.2f}"],
    ...
    resumo_norma = f"<b>Métricas NBR 14653:</b> R² = {model_stats['r2']} | <b>Fundamentação:</b> {model_stats['fundamentacao']} | <b>Precisão:</b> {model_stats['precisao']} | Amostras = {model_stats['saneadas']}."
    story.append(Paragraph(resumo_norma, text_style))
    story.append(Spacer(1, 5.0))
    story.append(Paragraph(f"<b>Equação Linear Estimada:</b> {equacao}", text_style))
    story.append(Spacer(1, 8.0))
...
st.markdown("Gestão automatizada de risco imobiliário por Inteligência Artificial em conformidade com a NBR 14653.")
st.divider()
...
with aba_avm:
    ...
    # Processamento seguro da planilha apenas se ela for de fato enviada
    df_global = None
    if arquivo_planilha is not None:
        try:
            ...
            st.error(f"Erro na leitura do arquivo enviado: {e}. Revertendo para dados padrão.")
    ...
    # Definição estrita das 5 variáveis específicas por tipologia
