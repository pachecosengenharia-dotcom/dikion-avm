...
def gerar_grafico_mercado(df_saneado, area_alvo, valor_estimado_m2):
    ...
    # Evitar divisão por zero caso venha dado zerado
    areas = df_saneado['area_principal_plot'].replace(0, 1)
    precos_m2 = df_saneado['valor_total_declarado'] / areas
    
    ax.scatter(df_saneado['area_principal_plot'], precos_m2, color='#2B6CB0', alpha=0.7, label='Amostras')
...
def gerar_laudo_pdf_ia(tenant, tipologia, area, valores, model_stats, status_juridico, score_juridico, grafico_buf, equacao=""):
    ...
    text_style = ParagraphStyle('T3', parent=styles['Normal'], fontSize=9, leading=13, spaceAfter=5.0)
    code_style = ParagraphStyle('T4', parent=styles['Normal'], fontSize=8, leading=10, textColor=colors.HexColor("#2D3748"), spaceAfter=5.0)
    
    story.append(Paragraph("LAUDO TÉCNICO DE ENGENHARIA DE AVALIAÇÕES POR IA", title_style))
    story.append(Paragraph(f"<b>Instituição Solicitante:</b> {tenant} | <b>Normativa:</b> ABNT NBR 14653", text_style))
    ...
    # Adicionado os Graus de enquadramento da norma no PDF
    story.append(Paragraph(f"<b>Métricas Estatísticas:</b> Coeficiente R² = {model_stats['r2']} | Amostras Processadas = {model_stats['saneadas']}", text_style))
    story.append(Paragraph(f"<b>Enquadramento NBR 14653:</b> Grau de Fundamentação: <b>{model_stats['fundamentacao']}</b> | Grau de Precisão: <b>{model_stats['precisao']}</b>", text_style))
    if equacao:
        story.append(Paragraph(f"<b>Equação Linearizada de Comportamento:</b>", text_style))
...
st.markdown("Gestao automatizada de risco imobiliario por Inteligencia Artificial baseada na NBR 14653.")
st.divider()
...
with aba_avm:
    ...
    # Estrutura de Dicionários de mapeamento categórico
