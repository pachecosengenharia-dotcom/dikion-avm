...
def gerar_graficos_diagnostico(y_real, y_pred, v1_alvo, val_medio):
    ...
    ax1.scatter(y_real, y_pred, color='#2B6CB0', alpha=0.7, edgecolors='k', label='Amostras')
    ax1.plot([y_real.min(), y_real.max()], [y_real.min(), y_real.max()], 'r--', lw=1.5, label='Linha de Identidade')
    ax1.set_title('Gráfico de Aderência (Real vs Predito)', fontsize=8, fontweight='bold')
    ax1.set_xlabel('Valores Reais (R$)', fontsize=7)
    ax1.set_ylabel('Valores Preditos (R$)', fontsize=7)
    ax1.grid(True, linestyle='--', alpha=0.5)
    ax1.legend(fontsize=6)
    
    # 2. Gráfico de Distribuição de Resíduos
    residuos = y_real - y_pred
    ax2.scatter(y_pred, residuos, color='#319795', alpha=0.7, edgecolors='k', label='Resíduos')
    ax2.axhline(y=0, color='r', linestyle='--', lw=1.5)
    ax2.set_title('Distribuição de Resíduos', fontsize=8, fontweight='bold')
    ax2.set_xlabel('Valores Preditos (R$)', fontsize=7)
    ax2.set_ylabel('Erros / Resíduos (R$)', fontsize=7)
...
def gerar_pdf(tenant, tipo, area, valores, stats, status_jur, score_jur, equacao, grafico_buf):
    buffer = io.BytesIO()
    ...
    story.append(Spacer(1, 10))
    story.append(Image(grafico_buf, width=460, height=140))
...
arquivo_planilha = col_up1.file_uploader("Upload da Planilha Comparativa (.xlsx ou .csv) [Opcional]", type=["xlsx", "csv"])
arquivo_certidao = col_up2.file_uploader("Upload da Certidão de Ônus / Matrícula (.pdf, .png, .jpg) [Opcional]", type=["pdf", "png", "jpg"])

if arquivo_planilha is not None: st.sidebar.success("🟩 Planilha Vinculada!")
...
# ==========================================
# 3. COMPUTAÇÃO E TRATAMENTO REGULATÓRIO
# ==========================================
if botao_calcular:
    try:
        ...
        y_predito_total = model.predict(X)
        ...
        # Geração de Gráficos de Aderência e Resíduos
        buf_graficos = gerar_graficos_diagnostico(y, y_predito_total, float(v1), val_medio)
