...
# PARAMETRIZAÇÃO DA PLANILHA (UPLOAD DIRETOR)
st.subheader("1. Coleta de Comparáveis")
...
if arquivo_planilha is not None:
    try:
        ...
        st.success(f"🟩 PLANILHA VALIDADA COM SUCESSO: {len(df_global)} registros carregados!")
    except Exception as e:
        st.error(f"Erro na estruturação do arquivo: {e}")

st.write("---")

# PARAMETRIZAÇÃO DOS INPUTS
st.subheader("2. Características do Imóvel Alvo")
tipologia_sel = st.selectbox("🎯 Selecione a Tipologia:", ["CASA", "APARTAMENTO", "LOTE", "GALPAO"])
...
if tipologia_sel == "CASA":
    ...
    v2 = col2.number_input("Área do Terreno (m²)", min_value=10.0, value=1000.0, key="gal_v2")
    v3 = col3.number_input("Índice Fiscal da Quadra", min_value=0.0, value=900.0, key="gal_v3")
